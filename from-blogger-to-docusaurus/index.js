...
const bloggerXmlPath = './blog-06-01-2025.xml';
const docusaurusDirectory = '../';
const notMarkdownable = [];
...
async function deleteExistingFiles(directory) {
 ...
 */
async function makeAuthorsYml(directory) {
    const authorsYml = `${author}:
  name: ${author_name}
  url: ${author_url}
  image_url: ${author_image_url}
`;
    await fs.promises.writeFile(path.join(directory, 'authors.yml'), authorsYml, 'utf-8');
}
async function getPosts() {
    ...
    const postsRaw = blog.feed[0].entry.filter((entry) => entry.category.some((category) => category.attr['@_term'] ===
        'http://schemas.google.com/blogger/2008/kind#post') &&
        entry.link.some((link) => link.attr['@_href'] && link.attr['@_type'] === 'text/html' && link.attr['@_rel'] === 'alternate'));
    const posts = postsRaw.map((entry) => {
        return {
            title: entry.title[0]['#text'],
            content: entry.content[0]['#text'],
            published: entry.published,
            link: entry.link.find((link) => link.attr['@_href'] && link.attr['@_type'] === 'text/html' && link.attr['@_rel'] === 'alternate')
                ? entry.link.find((link) => link.attr['@_href'] && link.attr['@_type'] === 'text/html' && link.attr['@_rel'] === 'alternate').attr['@_href']
                : undefined,
            tags: Array.isArray(entry.category) &&
                entry.category.some((category) => category.attr['@_scheme'] === 'http://www.blogger.com/atom/ns#')
                ? entry.category
                    .filter((category) => category.attr['@_scheme'] ===
                    'http://www.blogger.com/atom/ns#' &&
                    category.attr['@_term'] !== 'constructor') // 'constructor' will make docusaurus choke
                    .map((category) => category.attr['@_term'])
                : [],
        };
    });
    for (const post of posts) {
...
async function makePostIntoMarkDownAndDownloadImages(post) {
    const converter = new showdown.Converter({
        ghCodeBlocks: true,
    });
    const linkSections = post.link.split('/');
    const linkSlug = linkSections[linkSections.length - 1];
    const blogdirname = post.published.substring(0, 10) + '-' + linkSlug.replace('.html', '');
...
${markdown}
`;
    await fs.promises.writeFile(path.resolve(docusaurusDirectory, 'blog', blogdirPath, 'index.md'), content);
}
